...
def get_lines(filename, glue = False):
    if glue:
        return get_lines_glue(filename) #Hacky but less destructive to existing code...
    f = open(filename)
    lines = f.readlines()
    if "|||" in lines[0]:
        # remove the tag
...
def get_lines_glue(glue_filename):
    data = pd.read_csv(glue_filename, sep = '\t')
    cols = list(data)
    lines = data['text_a'].tolist()
    if 'text_b' in cols:
        lines.extend(data['text_b'].tolist())
    lines = [line.strip().lower() for line in lines if not isinstance(line, float)]
    return lines



def create_vocab(filename, tc_dir, background_train=False, cv_path="", glue = False, char_limit = 70):
    global w2i, i2w, CHAR_VOCAB
    char2count = defaultdict(lambda: 0)
    lines = get_lines(filename, glue = glue)
    for line in lines:
    ...
    if char_limit is not None:
        char_freq_list = [(char, char2count[char]) for char in char2count]
        char_freq_list.sort(key = lambda x: x[1], reverse = True)
        CHAR_VOCAB = [char for (char, freq) in char_freq_list[:char_limit]]
    ...
    if not os.path.exists(os.path.join(tc_dir, 'vocab')):
        os.makedirs(os.path.join(tc_dir, 'vocab'))
    pickle.dump(dict(w2i), open(tc_dir + "/vocab/" + task_name + "w2i_" + str(WORD_LIMIT) + ".p", 'wb'))
    pickle.dump(dict(i2w), open(tc_dir + "/vocab/" + task_name + "i2w_" + str(WORD_LIMIT) + ".p", 'wb')) # don't think its needed
    pickle.dump(CHAR_VOCAB, open(tc_dir + "/vocab/" + task_name + "CHAR_VOCAB_ " + str(WORD_LIMIT) + ".p", 'wb'))
    return
...
def get_batched_input_data(lines, batch_size, rep_list=['swap'], probs=[1.0]):
    #shuffle(lines)
    if len(rep_list) == 0 or 'all' in rep_list:
        rep_list = ['swap', 'drop', 'add', 'key']
        probs=np.ones(len(rep_list)) / len(rep_list)
    total_len = len(lines)
    output = []
