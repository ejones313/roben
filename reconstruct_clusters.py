...
def parse_args():
    ...
    parser.add_argument('--clusterer_dir', type = str, required = True,
                        help = 'Directory where all the partial clusterers are stored...')
    return parser.parse_args()

def reconstruct_clusters(clusterer_dir):
	file_paths = [os.path.join(clusterer_dir, f) for f in os.listdir(clusterer_dir) if os.path.isfile(os.path.join(clusterer_dir, f))]
	...
	save_path = '{}.pkl'.format(clusterer_dir)
...
if __name__ == '__main__':
    args = parse_args()
    reconstruct_clusters(args.clusterer_dir)
    new_path = args.clusterer_dir + '.pkl'
